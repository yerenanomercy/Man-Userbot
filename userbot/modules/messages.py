...
async def fastpurger(purg):
    ...
    if purg.reply_to_msg_id is not None:
        ...
        return await purg.edit("`Balas Ke Pesannya Goblok ツ `")

    if msgs:
        await purg.client.delete_messages(chat, msgs)
    done = await purg.client.send_message(
        purg.chat_id, f"`Berhasil Menghapus Kenangan`\
        \nJumlah Kenangan Yang Dihapus {str(count)} Pesan")
    """
    if BOTLOG:
...
async def purgeme(delme):
    ...
    smsg = await delme.client.send_message(
        delme.chat_id,
        "`Berhasil Menghapus Kenangan,` " + str(count) + " `Kenangan Telah Dihapus ツ`",
    )
    """
    if BOTLOG:
        await delme.client.send_message(
            BOTLOG_CHATID,
            "`Telah Menghapus Pesan,` " + str(count) + " Pesan Telah Dihapus ツ`")
    """
    await sleep(2)
    i = 1
    await smsg.delete()
...
async def delete_it(delme):
    ...
    if delme.reply_to_msg_id:
        try:
            ...
            if BOTLOG:
                await delme.client.send_message(
                    BOTLOG_CHATID, "`Berhasil Menghapus Pesan ツ`")
            """
