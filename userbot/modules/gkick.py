...
async def get_user_from_event(event):
    ...
    if event.reply_to_msg_id and not len(args) == 2:
        ...
        if user.isnumeric():
            user = int(user)
        if not user:
            await event.edit(f"`{ALIVE_NAME} Berikan username,user id, atau reply chat penggunanya goblok!`")
            return
...
async def gspide(rk):
    ...
    if user:
        ...
        for i in testrk:
            try:
                ...
                a += 1
                await rkp.edit(f"`{ALIVE_NAME} Memproses Global Kick Jamet Goblok! Gkicked di {a} Group Chats..`")
    ...
    return await rkp.edit(f"`Berhasil GKicked` [{user.first_name}](tg://user?id={user.id}) `di {a} Group Chats..`")

CMD_HELP.update({
    "gkick": "\
`.gkick reason`\
\nPenggunaan: kick pengguna secara global dari semua bot Administrasi Grup di mana Anda berada"
})
