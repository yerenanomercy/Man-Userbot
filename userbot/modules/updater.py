...
async def deploy(event, repo, ups_rem, ac_br, txt):
    if HEROKU_API_KEY is not None:
        ...
        if heroku_app is None:
            ...
            return repo.__del__()
        await event.edit('`[HEROKU]: Dyno Man-Userbot Sedang Dalam Proses, Mohon Menunggu 5-10 Menit`'
                         )
        ups_rem.fetch(ac_br)
        ...
        if build.status == "failed":
            ...
            await event.edit("`Man-Userbot Berhasil Di Deploy!" "Sedang Merestart, Tunggu Sebentar.....`")
...
async def upstream(event):
    ...
    if conf is None and force_update is False:
        ...
        return await event.respond('✥ **Perintah Untuk Update Man-Userbot**\n >`.update now`\n >`.update deploy`\n\n__Untuk Meng Update Fitur Terbaru Dari Man-Userbot.__')
...
CMD_HELP.update({
    'update':
    ".update"
    "\nUsage: Untuk Melihat Pembaruan Terbaru Man-Userbot."
    "\n\n.update now"
    "\nUsage: Memperbarui Man-Userbot."
    "\n\n.update deploy"
    "\nUsage: Memperbarui Man-Userbot Dengan Cara Deploy Ulang."
})
